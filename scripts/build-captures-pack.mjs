...
let pending = [];
try {
  const pendingUrl = CAPTURE_USER ? q("/pending") + `&user=${encodeURIComponent(CAPTURE_USER)}` : q("/pending");
...
const INSTR =
  ...
  `Respond with ONLY a JSON object {"fr","en","note"}: fr = a COMPLETE grammatically-correct French ` +
  `sentence (not a fragment or phrase) ending with proper punctuation (. ! ? …); ` +
...
for (const c of pending) {
  try {
    ...
    if (card && card.fr && /[.!?…»]$/.test(card.fr.trim())) built.push({ card, id: c._id });
    else if (card && card.fr) console.error("Skipping fragment (no terminal punctuation):", card.fr);
  } catch (e) {
